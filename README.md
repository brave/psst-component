...
A set of scripts designed to inject directly into an isolated world alongside other Brave-injected scripts within the web page, enforcing a standardized suite of privacy configurations via the Privacy Settings Tuning Tool.
...
### Generating `psst.json`

The `out/psst.json` file is generated by running the `scripts/generate-psst-config.ts` script, which automatically executes before the `bundle:dev` and `bundle:prod` tasks via npm lifecycle hooks. This script instantiates every website's `*UserScript` class to extract metadata like URL patterns, versions, and filenames, then writes this data into the JSON file. The PSST host relies on `psst.json` to efficiently determine exactly which user script to inject on a given page without needing to parse the raw source files at runtime.

## Script Parameters (Host → Script Communication)

Both `user.js` and `policy.js` are injected as plain text into an isolated world — they have no module system and no way to receive arguments directly. Instead, the host **assigns `window.__bravePsstParams = {...};` before injecting the bundle**.

Each script parses `window.__bravePsstParams = {...};` via a `parseParams()` method (see `UserScriptBase`/`PolicyScriptBase` in `src/common/`), which:

1. Prefers `window.__bravePsstParams`.
2. For backward compatibility supports old way: (`params` variable)
...
The `if (__DEV__)` guard acts as a conditional switch that enables detailed logging and debugging logic only during development and testing, while ensuring those statements are completely stripped out of the production build. This optimization prevents the runtime evaluation of expensive argument computations and reduces the final bundle size, as minifiers can safely delete the entire logging block when `__DEV__` is false.
...
## Build and test running commands
